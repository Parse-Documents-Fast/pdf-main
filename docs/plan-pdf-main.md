...
4. Para descarga: pedirle el Markdown a `pdf-persistance`; si el cliente pide Markdown, devolverlo tal cual; si pide PDF, pasarlo por `pdf-converter` (ADR-0005).
...
Además del servidor HTTP, `pdf-main` corre un proceso en background consumiendo `queue:extraction-results`. Al recibir un resultado, llama a `pdf-persistance` (HTTP, sin cambios) para actualizar el registro.
...
La comunicación con `pdf-validator` (gatilla la decisión de ruteo del propio `pdf-main`), con `pdf-converter` (solo en descarga a PDF, el cliente espera el archivo en esa misma conexión), y con `pdf-persistance`, sigue siendo HTTP síncrono.
